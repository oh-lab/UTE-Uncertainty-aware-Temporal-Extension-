...
from DDPG.ute_bandit import DDPG as UTE_DDPG
...
def eval_policy(policy, env_name, seed, eval_episodes=10, FiGAR=False, TempoRL=False, UTE=False):
    ...
    for _ in range(eval_episodes):
        ...
        if UTE:
            # get index from ucb
            j = policy.ucb.pull_index()
            # get beta gamma
            policy.uncertainty_factor = policy.lambdas[j]         
        ...
        while not done:
            if FiGAR:
                ...
                action = policy.select_action(np.array(state))
                repetition = policy.select_skip(np.array(state), action) + 1
            ...
            avg_decs += 1
            # print(repetition, avg_decs, avg_steps)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out-dir',
                        default='experiments/DDPG/',
                        type=str,
                        help='Directory to save results. Defaults to tmp dir.')
    parser.add_argument("--policy", default="TempoRLDDPG")  # Policy name (DDPG, FiGARDDPG, TempoRLDDPG, or our ute_bandit)
    ...
    out_dir = experiments.prepare_output_dir(args, user_specified_dir=args.out_dir + args.env + '/max_skip_' + str(args.max_rep) + '/' + args.policy + '/',
    ...
    elif args.policy.startswith('ute_bandit'):
        kwargs['skip_dim'] = max_rep
        policy = UTE_DDPG(**kwargs)
    else:
    ...
    if 'TempoRL' in args.policy or 'ute' in args.policy:
    ...
    evaluations = [[0, *eval_policy(policy, args.env, args.seed, FiGAR='FiGAR' in args.policy,
                                    TempoRL='TempoRL' in args.policy, UTE='ute' in args.policy)]]
    ...
    while t < int(args.max_timesteps):
        ...
        if t < args.start_timesteps:  # Before learning starts we sample actions uniformly at random
            ...
            elif args.policy.startswith('TempoRL') or args.policy.startswith('ute'):
            ...
            if 'FiGAR' in args.policy:
            ...
            elif 'ute' in args.policy:
                # UTE does not interfere with the action policy and its exploration
                action = (
                        policy.select_action(np.array(state))
                        + np.random.normal(0, max_action * args.expl_noise, size=action_dim)
                ).clip(-max_action, max_action)
                if np.random.random() < args.expl_noise:
                    ...
                    # the skip policy uses bootstrapped exploration for learning
                    repetition = policy.select_skip(state, action) + 1
            else:
                # Standard DDPG
                action = (
                        policy.select_action(np.array(state))
        ...
        # Perform action
        skip_states, skip_rewards = [], []  # used to build the local conectedness graph
        for curr_skip in range(repetition):
            ...
            if t >= args.start_timesteps:
                if 'ute' in args.policy:
                    # print("train1")
                    policy.train(skip_replay_buffer, args.batch_size)
                    # print("train2")
            ...
            if done:
                ...
                if 'ute_bandit' in args.policy:
                    # Update UCB
                    policy.ucb.push_data(ucb_datas)
                    # get index from ucb
                    j = policy.ucb.pull_index()
                    # get beta gamma
                    policy.uncertainty_factor = policy.lambdas[j]     
            ...
            if (t + 1) % args.eval_freq == 0:
                avg_reward, avg_decs, avg_steps = eval_policy(policy, args.env, args.seed, FiGAR='FiGAR' in args.policy,
                                                    TempoRL='TempoRL' in args.policy, UTE='ute' in args.policy)
                
                # np.save(f"{out_dir}/{file_name}", evaluations)
                if 'ute_bandit' in args.policy:
                    uncertainty_factor = policy.uncertainty_factor                  
                else:
                    uncertainty_factor = 0
                eval_stats = dict(
                    training_steps=t+1,
                    avg_num_steps_per_eval_ep=float(np.round(avg_steps, 1)),
                    avg_num_decs_per_eval_ep=float(np.round(avg_decs, 1)),
                    avg_rew_per_eval_ep=float(np.round(avg_reward, 1)),
