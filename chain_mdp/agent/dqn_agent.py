...
class DQNAgent():
    ...
    def act(self, state):
        ...
        #print("{}, {}, {}".format(torch.sum(state), state, self.online_net(state).data ))
    ...
    def act_extended_greedy(self, state, action=0, scaler=10, p=0.1, n=0):
        if n == 0:
        # draw from negative binomial distribution
            self.r_extend += 1
            n = max(1, int(self.input_dim*scaler/(np.random.negative_binomial(self.r_extend, p)+1)))
